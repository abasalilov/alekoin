...
contract("ERC223", accounts => {
  ...
  it("should change frozen status", async() => {
    ...
    it("should throw an error when trying to transfer more than balance", async function() {
        ...
        try {
            let transfer = await token.transfer(accounts[1], 100000000000000000011 ,{from: firstAccount});
        } catch(error) {
    ...
    it("should throw an error when trying to transfer more than allowed", async function() {
        ...
        try {
            ...
            let transfer = await token.transferFrom(firstAccount, accounts[1], 101, {from: accounts[1]});
        } catch (error) {
            console.log("Caught - Expected : " + error);
